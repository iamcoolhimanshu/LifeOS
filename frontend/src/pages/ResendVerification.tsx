...
export const ResendVerification: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/api/auth/resend-verification?email=${encodeURIComponent(email)}`);
      setMessage(response.data.message || 'Verification email sent successfully! Please check your inbox.');
    } catch (err: any) {
      setError(err.response?.data?.message || 'Something went wrong. Please wait a moment and try again.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen w-screen flex items-center justify-center bg-slate-50 dark:bg-[#050811] p-4 select-none">
      ...
      <GlassCard className="w-full max-w-md p-8 border border-slate-200 dark:border-slate-800/40 relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-xl font-black text-slate-800 dark:text-white tracking-wide uppercase">Resend Verification</h2>
          <p className="text-xs text-slate-500 mt-2">
            Enter your registered email below to receive a new activation link.
          </p>
        </div>
        ...
        {message ? (
          <div className="space-y-6">
            ...
            <Link
              to="/login"
              className="w-full py-2.5 px-4 rounded-xl text-xs font-bold bg-slate-100 border border-slate-200 dark:bg-slate-900 dark:border-slate-800 text-slate-700 dark:text-slate-450 hover:text-white transition-all text-center block"
            >
              Back to Login
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div className="p-3 rounded-xl border border-red-500/20 bg-red-500/10 text-red-400 text-xs font-semibold text-center flex items-center justify-center gap-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-900 text-xs text-slate-850 dark:text-white pl-10 pr-4 py-3 outline-none focus:border-cyberBlue focus:ring-1 focus:ring-cyberBlue/50 transition-all"
                />
            ...
            >
              {isLoading ? (
                ...
                <>
                  Request Activation Link <ArrowRight size={14} />
                </>
              )}
            </button>

            <div className="text-center mt-6">
              <Link to="/login" className="text-xs text-slate-500 hover:text-slate-800 dark:hover:text-cyberBlue transition">
